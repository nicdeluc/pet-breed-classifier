...
def get_pet_data():
        ...
        [
            ...
            transforms.Normalize(
                mean=[
                    ...
                    0.456,
                    0.406,
                ],  # Normalization of ImageSet (necessary if we use a model pre-trained on ImageSet, such as ResNet-18)
    ...
    train_data = OxfordIIITPet(
        root="data", split="trainval", transform=train_transform, download=True
    )
...
def fine_tune(
    model, dataloaders, loss_fn, optimizers, epochs_head=5, epochs_full=5, device="cuda"
):
    """ """
    ...
    history_head = train_model(
        model, dataloaders, loss_fn, optimizer_head, epochs=epochs_head, device=device
    )
    ...
    history_full = train_model(
        model, dataloaders, loss_fn, optimizer_full, epochs=epochs_full, device=device
    )
    print("Finished Training")

    # Join the histories
    history = {
        "train_loss": history_head["train_loss"] + history_full["train_loss"],
